...
func CreateStableVeth(hostIf, ifName, netnsPath string) (*string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to open target netns: %w", err)
	}
	defer ns.Close()
	...
	if err := netlink.LinkSetUp(hostLink); err != nil {
		return nil, fmt.Errorf("failed to bring up host link: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("peer link lookup failed: %w", err)
	}
	if err := netlink.LinkSetNsFd(peerLink, int(ns)); err != nil {
		return nil, fmt.Errorf("failed to move peer to target netns: %w", err)
	}

	// 4. Configure container side
	if err := netns.Set(ns); err != nil {
		return nil, fmt.Errorf("failed to switch to target netns: %w", err)
	}

	peerLink, err = netlink.LinkByName(peerIf)
	if err != nil {
		return nil, fmt.Errorf("failed to find peer in container ns: %w", err)
	}

	if err := netlink.LinkSetName(peerLink, ifName); err != nil {
		return nil, fmt.Errorf("failed to rename peer: %w", err)
	}

	if err := netlink.LinkSetUp(peerLink); err != nil {
		return nil, fmt.Errorf("failed to bring up peer: %w", err)
	}
